...
const LOCAL_WS_PORT = 8767; // 로컬 WebSocket 서버 포트 (모바일 앱 연결용)
...
const localWSServer = new WebSocket.Server({ port: LOCAL_WS_PORT });
...
console.log(`🌐 Relay Server: ${RELAY_SERVER_URL}`);
console.log(`🌐 Local HTTP: http://${localIP}:${HTTP_PORT}`);
console.log(`🔗 Extension WebSocket: ws://localhost:${EXTENSION_WS_PORT}`);
console.log(`📱 Local Mobile WebSocket: ws://${localIP}:${LOCAL_WS_PORT}`);
console.log(`\n💡 Usage:`);
console.log(`   - Local mode: Connect mobile app to ws://${localIP}:${LOCAL_WS_PORT}`);
console.log(`   - Relay mode:`);
console.log(`     - Create new session: curl -X POST http://localhost:${HTTP_PORT}/session/create`);
console.log(`     - Connect to session: curl -X POST http://localhost:${HTTP_PORT}/session/connect -H "Content-Type: application/json" -d '{"sessionId": "ABC123"}'`);
console.log(`   - Check status: curl http://localhost:${HTTP_PORT}/status\n`);
