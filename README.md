...
| **원격 모드** | 인터넷을 통한 외부 접속 | 포트 포워딩 또는 릴레이 서버 |
...
PC와 모바일이 다른 네트워크에 있을 때 사용합니다.

#### 방법 1: 포트 포워딩

라우터에서 포트 포워딩을 설정하여 외부에서 접속할 수 있습니다.
...
2. **포트 포워딩 설정**:
   - 외부 포트: `8767`
   - 내부 IP: PC의 로컬 IP (예: `192.168.0.10`)
   - 내부 포트: `8767`
   - 프로토콜: `TCP`
3. **공인 IP 확인**:
   ```bash
...
- 포트 포워딩은 보안 위험이 있을 수 있습니다
- 가능하면 VPN 사용을 권장합니다
- 사용하지 않을 때는 포트 포워딩을 비활성화하세요

#### 방법 2: SSH 터널링

더 안전한 방법으로, SSH를 통해 터널을 생성합니다.

**설정 단계:**

1. **PC에서 SSH 서버 활성화**
2. **모바일에서 SSH 클라이언트 사용** (예: Termius, JuiceSSH)
3. **터널 생성**:
   ```bash
   ssh -L 8767:localhost:8767 user@your-pc-public-ip
   ```
4. **모바일 앱에서 `localhost:8767`로 연결**

#### 방법 3: 릴레이 서버 (향후 지원 예정)

클라우드 릴레이 서버를 통한 연결을 지원할 예정입니다.
