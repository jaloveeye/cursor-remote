...
| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| 1 | **Extension 0.3.1 마켓플레이스 배포** | [ ] | vsix 준비됨. PAT·퍼블리시만 진행 |
| 2 | **클라이언트 웹 재배포 (Vercel)** | [ ] | 로컬 `flutter build web` 후 `build/web`에서 `vercel --prod` |
| 3 | **릴레이 서버 운영 점검** | [ ] | 필요 시 Redis·헬스 체크 확인 |
...
| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| 1 | **포트/권한 에러 처리** | [ ] | 8766 사용 불가 시 명확한 메시지·재시도 또는 종료 |
| 2 | **릴레이 끊김 시 재연결** | [ ] | 세션 끊어졌을 때 Extension·앱 쪽 재연결·상태 표시 |
| 3 | **에러 메시지 정리** | [ ] | 자주 나오는 에러별 안내 문구 정리 (USER_MANUAL 등) |
...
| 2 | **실시간 로그 표시** | [ ] | Extension 로그를 앱에서 필터·검색 가능하게 |
