...
**최종 수정**: 2026-02-02 (우선순위·실행 계획 정리, 0.3.8 반영)

---

## ✅ 최근 완료 (참고)

- **Extension 0.3.8 마켓플레이스 배포** (2026-02-02): ✅ 완료
- **0.3.8 릴레이·세션 정규화** (2026-02-02):
  - 릴레이 모드 "응답 대기 중" 표시: 전송 후 UI가 먼저 그려지도록 보장, 파싱 실패 시 대기 상태 유지
  - 채팅 히스토리 세션 격리: `getChatHistory`에 `relaySessionId` 필터 추가, 모바일에서 현재 세션 히스토리만 로드
  - 메시지 개수: "표시 / 전체"를 사용자 프롬프트+AI 응답만 카운트하도록 변경
  - 빈 메시지 영역 문구: "메시지가 없습니다" / "검색 결과가 없습니다"로 구분
  - 채팅 캡처: `output` 스킴 문서 제외로 "Initial content length: 0" 반복 로그 제거
- **릴레이 서버 connect 500 수정** (2026-02-02):
  - `connect.ts`에서 `PC_STALE_MS` 중복 선언 제거
  - `poll.ts`/`send.ts`에서 `sessionId`를 `trim().toUpperCase()`로 정규화해 "Session not found" 해소
- **0.3.6 세션 관리 개선**:
  - 세션 ID 입력/저장: 익스텐션 시작 시 6자리 세션 ID 입력, globalState에 저장
  - PC 먼저 연결 가능: PC가 세션 ID 입력 시 해당 ID로 세션 자동 생성
  - Heartbeat: PC가 30초마다 heartbeat 전송, 2분간 없으면 연결 끊김으로 간주
...
아래는 **지금부터의 작업 순서**를 정리한 실행 계획이다. 번호가 작을수록 먼저 진행 권장.

| 순서 | 작업 | 우선순위 | 예상 부담 | 비고 |
|------|------|----------|-----------|------|
| 1 | **Extension 0.3.8 마켓플레이스 배포** | 🔴 높음 | 낮음 | ✅ 완료 |
| 2 | **릴레이 서버 운영 점검 (정기)** | 🔴 높음 | 낮음 | connect/poll/send 로그·에러 모니터링, Redis 상태 확인 |
| 3 | **AI 응답 스트리밍 (선택)** | 🟡 중간 | 중간 | 설계 후 릴레이·앱 연동. 당장 필수는 아님 |
| 4 | **이미지 생성 결과 안내 (2.4)** | 🟢 낮음 | 낮음 | 응답에 assets 경로 포함 여부 확인 후, 모바일 표시 여부 결정 |
| 5 | **TOTP(Google Authenticator) 연동** | 🟢 보안 | 중간 | PIN 대안. 설계·구현 순서: 릴레이 → 익스텐션 → 모바일 |
| 6 | **파일 편집 / 권한 플로우** | 🟣 선택 | 높음 | 읽기·쓰기 API·권한·UI. 필요 시 별도 Phase로 설계 |

**Phase별 요약**

- **Phase 1 (즉시)**: 0.3.8 배포, 릴레이 정기 점검
- **Phase 2 (단기)**: 스트리밍·이미지 안내 등 UX 개선 검토
- **Phase 3 (중기)**: TOTP 도입 검토
- **Phase 4 (장기)**: 파일 편집·권한 플로우 등 대형 기능
...
| 2 | **Extension 0.3.8 마켓플레이스 배포** | [x] | ✅ 완료 |
| 3 | **클라이언트 웹 재배포 (Vercel)** | [x] | ✅ 완료 |
| 4 | **릴레이 서버 운영 점검** | [x] | ✅ 완료. 정기 점검 절차: [relay-server/MAINTENANCE.md](relay-server/MAINTENANCE.md) |
...
| 3 | **연결 설정 단순화** | [x] | ✅ 완료 - 세션 ID 입력/저장, globalState에 저장하여 재사용 |
...
1. Cursor 2.4 + Cursor CLI 설치 환경에서
   - [x] Extension → `agent -p`, `--mode plan/ask`, `--resume` 호출로 채팅 전송
   - [x] 모바일(또는 웹) → 릴레이 → Extension → CLI 흐름으로 한 번에 질문·추가 질문 보내기
   - [x] Clarification Questions 발생 시, 모바일에서 답변 메시지 보냈을 때 같은 세션에 반영되는지 확인

2. 문서
   - [x] "Cursor 2.4 사용 시" 짧은 문단 추가 (USER_MANUAL 또는 README). Skills/Subagents는 기본 동작, 원격에서도 동일하게 적용됨을 명시
...
| # | 작업 | 상태 | 비고 |
|---|------|------|------|
| 1 | **TOTP(Google Authenticator) 연동** | [ ] 계획 | PIN보다 강한 보안. PC 활성화 시 TOTP secret 생성·QR 표시 → 모바일 접속 시 6자리 코드 검증. 릴레이: otplib 등, 익스텐션: QR 표시, 모바일: TOTP 입력 UI. PIN과 선택 옵션으로 둘 수 있음. |
