...
let terminalOutputFile: string | null = null;
let lastTerminalOutputSize: number = 0;
// 터미널별 출력 리스너 관리
const terminalDataListeners: Map<vscode.Terminal, vscode.Disposable> = new Map();
...
// 터미널 출력 파일 모니터링 시작
function startTerminalOutputFileMonitoring(context: vscode.ExtensionContext) {
    // 워크스페이스 루트 경로 가져오기
...
function setupRulesBasedChatCapture(context: vscode.ExtensionContext) {
...
let chatDocumentMonitorInterval: NodeJS.Timeout | null = null;
let lastChatContent: string = '';
let currentChatUri: vscode.Uri | null = null;
let isProcessingChatContent: boolean = false;
let lastProcessedContentHash: string = '';
let processDebounceTimer: NodeJS.Timeout | null = null;

function startChatDocumentMonitoring(context: vscode.ExtensionContext) {
    // 모든 열린 문서 모니터링 (디버깅용)
    ...
    chatDocumentMonitorInterval = setInterval(() => {
        ...
        try {
            const document = vscode.workspace.textDocuments.find(doc => 
                doc.uri.toString() === currentChatUri!.toString()
            );
...
function processNewChatContent(newContent: string, fullContent: string) {
    // 처리 중 플래그 설정
    if (isProcessingChatContent) {
        return;
    }
...
// Rules 파일 생성/확인
function ensureRulesFile(workspaceRoot: string) {
...
function ensureHooksFile(workspaceRoot: string) {
    ...
    try {
        ...
        if (fs.existsSync(hooksFile)) {
            try {
                ...
                const hasAfterAgentResponseHook = existingContent.hooks && existingContent.hooks.some((h: any) => 
                    (h.event === 'afterAgentResponse' || h.event === 'agent_message') && 
                ...
                if (hasAfterAgentResponseHook) {
                    ...
                    const existingHook = existingContent.hooks.find((h: any) => 
                        (h.event === 'afterAgentResponse' || h.event === 'agent_message') && 
...
function startChatFileWatcher(context: vscode.ExtensionContext, workspaceRoot: string) {
    const cursorDir = path.join(workspaceRoot, '.cursor');
...
function findAvailablePort(startPort: number, maxAttempts: number = 10): Promise<number | null> {
...
// 터미널 출력 모니터링 시작
function startTerminalOutputMonitoring(context: vscode.ExtensionContext) {
    // 기존 리스너가 있으면 제거
    if (terminalOutputListener) {
        terminalOutputListener.dispose();
    }
