...
- 모바일 앱을 수정하여 Extension의 WebSocket 서버(포트 8766)에 직접 연결하도록 변경하면 완전히 동작
- 또는 모바일 앱에서 로컬 IP 주소와 포트 8766을 입력하여 연결
...
2. **모바일 클라이언트 연결**
   - 모바일 앱에서 "로컬 모드" 선택
   - PC의 로컬 IP 주소 입력 (예: `192.168.0.10`)
   - 포트: `8766` (Extension의 WebSocket 서버 포트)
   - 연결 버튼 클릭
...
**참고**: 모바일 앱이 현재 포트 8767에 연결하도록 되어 있다면, 포트를 8766으로 변경하거나 모바일 앱을 수정해야 합니다.
...
1. **로컬 모드**: 모바일 앱이 포트 8766에 연결하도록 수정 필요 (현재는 포트 8767로 설정됨)
2. **네트워크 의존성**: 원격 모드 사용 시 릴레이 서버 접근 필요
3. **세션 감지 시간**: 원격 모드에서 최대 10초 소요 (폴링 간격 2초)
